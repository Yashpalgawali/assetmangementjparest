...
public interface AssetRepo extends JpaRepository<Assets, Long> {
	...
	// @Query(value="SELECT * FROM tbl_assets JOIN tbl_assettype ON
	// tbl_assettype.type_id=tbl_assets.type_id ",nativeQuery = true)
...
//	public int getAssetQuantityByAssetId(Long asid);
	...
	@Query("SELECT SUM(a.quantity) FROM Assets a")
	public int sumOfTotalAssetsQuantity();
}
